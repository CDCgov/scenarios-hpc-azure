...
Note: --download_dir must be specified to automatically download files
after job complete, --targets is optional but conditional on --download_dir]
being specified."""
...
parser.add_argument(
    ...
    required=False,
    metavar="",
    help="directory on local to output blob storage files generated by this job to",
)
...
parser.add_argument(
    ...
    nargs="*",
    required=False,
    help="specific file or directory names you wish to download, defaults to "
    "entire job's outputs if --download_dir specified",
)
...
def launch():
    ...
    dest_path: str = args.download_dir
    ...
    if dest_path is not None:
        if os.path.exists(dest_path):
            launcher.monitor_and_download(
                timeout_mins, dest_path, targets=targets
            )
        else:
            print(
                (
